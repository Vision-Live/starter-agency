...
<div class="fixed top-0 left-0 w-screen h-screen  transition-all duration-300 {isDrawerOpen ? 'backdrop-blur-sm bg-black/30' : 'z-auto pointer-events-none'}">
	<div use:clickOutside on:click_outside={isDrawerOpen ? closeDrawer : ''} class="{isDrawerOpen ? '' : '-translate-x-[100%]'} pr-16 pl shadow-lg shadow-base-300 w-fit h-full bg-base-100 flex flex-col justify-start items-center transition-all duration-300">
